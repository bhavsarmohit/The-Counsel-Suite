...
if ($result->num_rows > 0) {
  ...
  while($row = $result->fetch_assoc()) {
     $filepath="../../documents/marketing/".$row["c_file"];
  }
} 
...
$reterive_legal_count = "SELECT * FROM marketing_downloads WHERE date='$date'";
...
if ($result2->num_rows > 0) {
  ...
  while($row = $result2->fetch_assoc()) {
     $total_download_count=$row["downloads"];
     $total_download_count=$total_download_count+1;
     $update_count="UPDATE marketing_downloads SET downloads='$total_download_count' WHERE date='$date'";
     $mysqli->query($update_count);
  }
...
<script>
  function goback()
  {
    window.location.replace('view_marekting_favorites.php');
  }
  </script>
